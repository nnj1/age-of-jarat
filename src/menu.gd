...
#@onready var host_resources = $TabContainer/Host/StartingRes
#@onready var host_unit_cap = $TabContainer/Host/UnitCap
...
func _ready():
	...
	#host_resources.value = MultiplayerManager.server_settings["starting_resources"]
	#host_unit_cap.value = MultiplayerManager.server_settings["unit_cap"]
...
func _on_host_button_pressed():
	...
	MultiplayerManager.server_settings["starting_resources"] = 0
	MultiplayerManager.server_settings["unit_cap"] = 9998
...
func _update_local_identity():
	...
	if final_name.is_empty(): final_name = "Commander_" + str(randi() % 1000)
